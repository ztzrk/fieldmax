...
export default function FieldDetailPage() {
    ...
    const handleBook = () => {
        if (!user) {
            ...
            // TODO: Implement booking modal or redirection to checkout
            // For now, maybe just show a toast or nothing as Per instruction "direct to field detail page"
            // But if they are ON the detail page, "Book Now" should probably do something.
            // Since "Book Now" placeholder exists on home, I'll keep it simple here.
                    ...
                    <div className="overflow-hidden rounded-xl border bg-muted aspect-video relative">
                        {field.photos && field.photos.length > 0 ? (
                            <Carousel className="w-full h-full">
                                <CarouselContent>
                                    {field.photos.map((photo: { url: string; id: string }) => (
                ...
                <div className="flex flex-col gap-6">
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            {field.isClosed && <Badge variant="destructive">Closed</Badge>}
                        </div>
                        <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2">{field.name}</h1>
                    ...
                    <div className="flex flex-col gap-1 p-6 bg-muted/30 rounded-lg border">
                        <span className="text-sm text-muted-foreground font-medium uppercase tracking-wider">Price</span>
                        <div className="flex items-baseline gap-1">
                            <span className="text-3xl font-bold text-primary">{formatPrice(field.pricePerHour)}</span>
                    ...
                    <div className="pt-6 mt-auto">
                        <Button size="lg" className="w-full text-lg h-12 gap-2" onClick={handleBook}>
